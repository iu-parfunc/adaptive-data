...
module Control.Concurrent.Adaptive.MichaelScottQueue
 (
   -- The convention here is to directly provide the concrete
   -- operations as well as providing the typeclass instances.
   LinkedQueue(), newQ, nullQ, pushL, tryPopR, freeze
 )
  where

import Data.Concurrent.IORef (readIORef, newIORef, IORef, casIORef, readForCAS, peekTicket, freezeIORef)
import System.IO (stderr)
import Data.ByteString.Char8 (hPutStrLn, pack)
import Control.Monad (unless)

-- import GHC.Types (Word(W#))
import qualified GHC.IORef as IR
...
pairEq (Cons _ ref1) (Cons _ ref2) = ref1 == ref2
pairEq _ _ = False
...
freeze :: LinkedQueue a -> IO ()
freeze (LQ headPtr tailPtr) = go headPtr >> go tailPtr
  where
    go ptr = do
      freezeLoop ptr =<< readForCAS ptr
      pair <- readIORef ptr
      case pair of
        Null        -> return ()
        Cons _ ptr' -> go ptr'

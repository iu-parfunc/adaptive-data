...
-- import Data.Vector
import Control.Exception (SomeException, catch, evaluate, throw, try)
import Control.Monad     (forM_, unless, when)
import Text.Printf       (printf)

import Data.Atomics         (loadLoadBarrier, storeLoadBarrier, writeBarrier)
import Data.Atomics.Counter (AtomicCounter, casCounter, newCounter, peekCTicket,
                             readCounter, readCounterForCAS, writeCounter)
...
freeze :: ChaseLevDeque a -> IO ()
freeze CLD { top, bottom, activeArr } = freezeLoop activeArr =<< readForCAS activeArr
  where
